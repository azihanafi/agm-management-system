                ...
                <div>
                    ...
                    <select wire:model="selectedMemberId" class="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-indigo-500 outline-none">
                        ...
                        @foreach($members as $member)
                            <option value="{{ $member->id }}">{{ $member->name }} ({{ $member->staff_id }})</option>
                        @endforeach
                    </select>
                </div>
                            ...
                            <tr class="hover:bg-white/5 transition-all">
                                <td class="px-8 py-4">
                                    <div class="flex items-center gap-2">
                                        <button wire:click="moveUp({{ $official->id }})" class="p-1 hover:bg-white/10 rounded">
                                        ...
                                        <button wire:click="moveDown({{ $official->id }})" class="p-1 hover:bg-white/10 rounded">
                                ...
                                <td class="px-8 py-4">
                                    <div class="font-bold text-sm">{{ $official->user->name }}</div>
                                    <div class="text-[10px] text-gray-500 uppercase">{{ $official->user->staff_id }} | {{ $official->user->workplace }}</div>
                                </td>
                                <td class="px-8 py-4 text-right">
                                    <button wire:click="removeOfficial({{ $official->id }})" class="text-red-400 hover:text-red-300">
    ...
    <div class="bg-white/5 border border-white/10 rounded-3xl p-6 md:p-10">
        ...
        <div class="flex flex-col items-center w-full max-w-6xl mx-auto">
            ...
            <div class="w-full flex justify-center">
                @foreach($officials->where('designation', 'PENGERUSI') as $p)
                    <div class="bg-indigo-600 p-1 rounded-2xl shadow-2xl shadow-indigo-500/20 w-full max-w-sm">
                        <div class="bg-[#0a0a0c] px-6 py-8 rounded-xl border border-white/10 text-center">
                            ...
                            <div class="font-bold text-lg md:text-xl uppercase leading-tight">{{ $p->user->name }}</div>
                            <div class="text-[10px] text-gray-500 mt-2 uppercase tracking-wider">{{ $p->user->workplace }}</div>
                        </div>
                    </div>
                @endforeach
            </div>
            ...
            <div class="w-full grid grid-cols-1 md:grid-cols-2 gap-8">
                @foreach($officials->whereIn('designation', ['SETIAUSAHA', 'BENDAHARI']) as $o)
                    <div class="flex justify-center">
                        <div class="bg-white/5 border border-white/10 p-1 rounded-2xl w-full max-w-sm relative">
                            <div class="absolute -top-3 left-1/2 -translate-x-1/2 bg-indigo-600 px-4 py-1 rounded-full text-[8px] font-black uppercase tracking-widest">{{ $o->designation }}</div>
                            <div class="bg-black/40 px-6 py-6 rounded-xl text-center">
                                <div class="font-bold text-sm md:text-base uppercase leading-tight mt-2">{{ $o->user->name }}</div>
                                <div class="text-[9px] text-gray-500 mt-2 uppercase">{{ $o->user->workplace }}</div>
                            </div>
                        </div>
                    </div>
                @endforeach
            ...
            <!-- Level 3: Other Roles (Timbalan, Penolong, Amanah, Juruaudit) -->
             <div class="w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
                @foreach($officials->filter(fn($o) => !in_array($o->designation, ['PENGERUSI', 'SETIAUSAHA', 'BENDAHARI']) && !str_contains(strtoupper($o->designation), 'AJK')) as $o)
                    <div class="bg-white/5 border border-white/10 px-6 py-6 rounded-2xl text-center flex flex-col justify-center min-h-[100px] hover:border-indigo-500/50 transition-all">
                        <div class="text-[9px] font-bold text-indigo-400 uppercase tracking-[0.2em] mb-2">{{ $o->designation }}</div>
                        <div class="font-bold text-xs md:text-sm uppercase leading-tight">{{ $o->user->name }}</div>
                        <div class="text-[8px] text-gray-600 mt-2 uppercase">{{ $o->user->workplace }}</div>
                    </div>
                @endforeach
            </div>
            ...
            <div class="w-full border-t-2 border-indigo-500/30 pt-12 relative">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                    @foreach($officials->filter(fn($o) => str_contains(strtoupper($o->designation), 'AJK')) as $o)
                        <div class="bg-white/5 border border-white/5 p-6 rounded-xl text-center hover:bg-white/10 transition-all flex flex-col justify-center min-h-[100px] relative group overflow-hidden">
                            ...
                            <div class="text-[8px] font-black text-indigo-400 uppercase tracking-widest mb-2">{{ $o->designation }}</div>
                            <div class="font-bold text-[11px] uppercase leading-snug">{{ $o->user->name }}</div>
                            <div class="text-[8px] text-gray-500 uppercase mt-2 italic">{{ $o->user->workplace }}</div>
                        </div>
                    @endforeach
                </div>
            </div>
