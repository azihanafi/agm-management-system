<div class="max-w-4xl mx-auto p-6" wire:poll.5s>
    <header class="flex justify-between items-center mb-10">
        ...
        <div class="flex items-center gap-3 bg-white/5 border border-white/10 px-4 py-2 rounded-full">
            <div class="w-2 h-2 {{ $isVotingOpen ? 'bg-green-500 animate-pulse' : 'bg-red-500' }} rounded-full"></div>
            <span class="text-[10px] font-black uppercase tracking-widest text-gray-400">
                {{ $isVotingOpen ? 'LIVE SESSION' : 'VOTING CLOSED' }}
            </span>
        </div>
    </header>
    ...
    @elseif($hasVoted)
        <div class="bg-white/5 border border-white/10 rounded-3xl p-16 text-center">
            ...
            <p class="text-gray-400">You have successfully cast your vote for the position of <strong>{{ $activePosition->title }}</strong>.</p>
    ...
    @else
        ...
        <div class="bg-white/5 border border-white/10 rounded-3xl p-8 backdrop-blur-xl shadow-2xl relative overflow-hidden">
            ...
            <div class="mb-10">
                ...
                <h2 class="text-4xl font-black text-white mt-4 uppercase tracking-tighter">{{ $activePosition->title }}</h2>
            </div>

            <div class="space-y-4">
                @foreach($activePosition->nominations as $nomination)
                    @php 
                        $isEligible = $nomination->isEligible();
                    @endphp
                    <div class="group relative flex items-center justify-between p-6 rounded-2xl border transition-all duration-300 {{ $isEligible ? 'bg-white/5 border-white/10 hover:border-indigo-500/50 hover:bg-white/[0.08]' : 'bg-black/20 border-white/5 opacity-50' }}">
                        <div class="flex items-center gap-6">
                            <div class="w-14 h-14 bg-white/5 rounded-xl flex items-center justify-center font-black text-xl text-gray-400 border border-white/10">
                                {{ substr($nomination->nominee->name, 0, 1) }}
                            </div>
                            <div>
                                <h3 class="text-xl font-bold text-white">{{ $nomination->nominee->name }}</h3>
                                <p class="text-gray-500 text-sm">{{ $nomination->nominee->workplace }}</p>
                            </div>
                        </div>

                        @if($isEligible)
                            <button wire:click="castVote({{ $activePosition->id }}, {{ $nomination->nominee->id }})" 
